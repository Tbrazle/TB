...
export default function Home() {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome */}
      <div className="card">
        <h1 className="text-2xl font-bold text-buildmode-900">
      ...
      {challenge && !challenge.completed && (
        <Link to="/daily-challenge" className="block card bg-gradient-to-r from-anchor-600 to-anchor-700 text-white hover:shadow-lg transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-buildmode-100 text-sm font-medium">Today's Challenge</p>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/habits" className="text-sm text-buildmode-600 hover:text-buildmode-800">Manage</Link>
        </div>
        {habits.length === 0 ? (
          <p className="text-gray-500 text-sm">No habits yet. <Link to="/habits" className="text-buildmode-600 underline">Add one</Link></p>
        ) : (
          <div className="space-y-2">
            <div className="flex gap-1">
              {Array.from({ length: habits.length }).map((_, i) => (
                <div key={i} className={`h-2 flex-1 rounded-full ${i < completedCount ? 'bg-buildmode-500' : 'bg-gray-200'}`} />
              ))}
            </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/lessons" className="text-sm text-buildmode-600 hover:text-buildmode-800">View all</Link>
        </div>
        <div className="space-y-3">
          {recentLessons.length === 0 ? (
            ...
            recentLessons.map(lesson => (
              <Link key={lesson.id} to={`/lessons/${lesson.slug}`} className="block p-3 rounded-xl bg-buildmode-50 hover:bg-buildmode-100 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  {lesson.progress?.completed ? (
                    <span className="text-buildmode-600">✓</span>
                  ) : (
      ...
      {/* Premium upsell */}
      {!isPremium && (
        <Link to="/pricing" className="block card border-2 border-buildmode-200 bg-gradient-to-br from-white to-anchor-50">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🌟</span>
            <div>
              <h3 className="font-semibold text-buildmode-800">Go Premium</h3>
